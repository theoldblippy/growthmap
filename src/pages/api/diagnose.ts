...
// ---- Cameron's rubric, encoded (deterministic base + LLM seed) ----
const STATIONS = [
...
type Fix = { move: string; why: string };
type Base = { verdict: string; read: string; fixes: Fix[] };
...
function pickConstraint(ratings: string[]): number {
  let i = ratings.findIndex((r) => r === 'r');
  if (i < 0) i = ratings.findIndex((r) => r === 'y');
  return i;
}

async function personalize(base: Base, ctx: any, apiKey: string): Promise<Base | null> {
